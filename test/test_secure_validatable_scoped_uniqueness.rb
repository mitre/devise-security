...
class TestSecureValidatableScopedUniqueness < ActiveSupport::TestCase
  ...
  # The scoped validator is registered first, so secure_validatable's
  # `uniqueness_validation_of_login?` must detect it and skip adding
  # its own unscoped validator.
  ...
  # secure_validatable should add one.
  ...
  # secure_validatable should detect it and skip adding its own.
  ...
  def uniqueness_validator_count(klass)
    orm_klass = DEVISE_ORM == :active_record ? ActiveRecord::Validations::UniquenessValidator : ::Mongoid::Validatable::UniquenessValidator
    klass.validators.count { |v| v.is_a?(orm_klass) && v.attributes.include?(:email) }
  end
  ...
  test 'scoped uniqueness validator preserves its original scope' do
    orm_klass = DEVISE_ORM == :active_record ? ActiveRecord::Validations::UniquenessValidator : ::Mongoid::Validatable::UniquenessValidator
    validator = ScopedUniquenessUser.validators.find { |v| v.is_a?(orm_klass) && v.attributes.include?(:email) }
